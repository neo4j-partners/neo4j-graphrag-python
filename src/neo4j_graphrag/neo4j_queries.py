...
def upsert_node_query_merge(
    ...
    # Build identity properties map for apoc.merge.node
    # Use dot notation for property access (works for standard property names)
    # For properties with special characters, backticks escape the identifier
    # Format: {`propertyName`: row.properties.`propertyName`}
    identity_props = (
        "{`" + merge_property + "`: row.properties.`" + merge_property + "`}"
    )
    # IMPORTANT: Merge on ONLY the primary entity label (first in row.labels), e.g., 'Company'.
